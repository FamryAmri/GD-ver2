...
class mainLib extends dbremote {
	...
	public function getUserID($extID, $userName = "Undefined") {
		...
		$usrID = $query->fetchColumn();
			...
			$rowCount = $this->dbremote_query ("SELECT userID FROM users WHERE extID = ". $extID, true);
			$usrID = $this->dbremote_query ("SELECT userID FROM users WHERE extID = ". $extID, false);
		...
		if ($rowCount > 0) {
			...
			$userID = $db->lastInsertId();
				...
				$userID = $this->dbremote_query ("INSERT INTO users (isRegistered, extID, userName, lastPlayed) VALUES ( ". $register.",". $extID.", '". $userName."',".time().")", false);
		}
	}
		return $userID;
	}
	...
	public function getUserName($userID) {
		...
		$rowCount = $query->rowCount();
		$nama = $query->fetchColumn();
		} elseif ($isDB == 2){
			$prepare = "SELECT userName FROM users WHERE userID ". $userID;
			$query = $this->dbremote_query ($prepare, false);
			$fetch = json_decode ($query)[0];
			$nama = $fetch->userName;
			$query2 = $this->dbremote_query ($prepare, true);
			$rowCount = $query2;
			}
		if ($rowCount > 0) {
			$userName = $nama;
	...
	public function getAccountIDFromName($userName) {
		...
		$extID = $query->fetchColumn();
			...
			$query = $this->dbremote_query ($prepare, true);
			$rowCount = $query;
			$query2 = $this->dbremote_query ($prepare, false);
			$extID = json_decode ($query)[0]->accountID;
			}
		if ($rowCount > 0) {
			$accountID = $extID;
		} else {
	...
	public function getExtID($userID) {
		...
		$accountID = $query->fetchColumn();
			...
			$rowCount = $this->dbremote_query ("SELECT extID FROM users WHERE userID = ". $userID, true);
			$accountID = $this->dbremote_query ("SELECT extID FROM users WHERE userID = ". $userID, false);
			$accountID = json_decode ($accountID)[0]->accountID;
			}
		if ($rowCount > 0) {
			return $accountID;
		}else{
	...
	public function getUserString($userID) {
		...
		$accountID = $userdata['extID'];
		$userName = $userdata['userName'];
		} elseif ($isDB == 2){
			$query = dbremote ("SELECT userName, extID FROM users WHERE userID = ". $userID, false);
			$query = json_decode ($query)[0];
			$accountID = $query->extID;
	...
	public function checkPermission($accountID, $permission){
		...
		$isAdmin = $query->fetchColumn();
			...
			$query = $this->dbremote_query("SELECT isAdmin FROM accounts WHERE accountID = ". $accountID, false);
			$isAdmin = json_decode ($query)[0]->isAdmin;
			}
		if($isAdmin == 1){
			return 1;
		}
	...
	public function rateLevel($accountID, $levelID, $stars, $difficulty, $auto, $demon){
		...
		$query->execute([':value' => $this->getDiffFromStars($stars)["name"], ':timestamp' => time(), ':id' => $accountID, ':value2' => $stars, ':levelID' => $levelID]);
			...
			$query = $this->dbremote_query("UPDATE levels SET starDemon = '". $demon.", starAuto = ". $auto.", starDifficulty = ". $difficulty.", starStars = ". $stars.", levelID = ". $levelID.", rateDate = ". time()." WHERE levelID = ". $levelID, false);
			$query = $this->dbremote_query("INSERT INTO modactions (type, value, value2, value3, timestamp, account) VALUES ('1', '". $this->getDiffFromStars($stars)['name']."',". $stars.",  ". $levelID.",". time().", ". $accountID.")", false);
		}
	...
	public function featureLevel($accountID, $levelID, $feature){
		...
		$query->execute([':value' => $feature, ':timestamp' => time(), ':id' => $accountID, ':levelID' => $levelID]);
			...
			$query = $this->dbremote_query ("UPDATE levels SET starFeatured = '".$feature."', rateDate = '". time()."' WHERE levelID = ".$levelID, false);
			$query = $this->dbremote_query ("INSERT INTO modactions (type, value, value3, timestamp, accountID) VALUES ( '2', '". $feature."', ". $levelID.", '". time()."', ". $levelID.")", false);
	}
}
	public function verifyCoinsLevel($accountID, $levelID, $coins){
		...
		$query = $this->dbremote_query ("UPDATE levels SET starCoins = ".$coins." WHERE levelID = ". $levelID, false);
		$insert = $this->dbremote_query ("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES ( '3', '". $coins."',". $levelID.",'". time()."',". $accountID."')", false);
		}
	}
	public function songReupload($url){
		...
		if (filter_var($song, FILTER_VALIDATE_URL) == TRUE) {
			...
			$count = $query->fetchColumn();
				...
				$query = $this->dbremote_query ("SELECT count(*) FROM songs WHERE download = '". $download."'", false);
				$count = json_decode ($query)[0]->ID;
				}
			if($count != 0){
				return "-3";
			}else{
				...
				return $db->lastInsertId();
					...
					$query = $this->dbremote_query ("INSERT INTO songs (name, authorID, authorName, size, download, hash) VALUES ('".$name."','9','".$author."','".$size."','".$download."','". $hash."'", true);
					if ($query == 1){
						$getID = $this->dbremote_query ("SELECT ID,download FROM songs WHERE download = '". $download."'", false);
						$getID = json_decode ($getID)[0]->ID;
						return $getID;
						} else {
							return "-3";
							}
	...
	public function getFileSize($url){
		...
		curl_setopt($chf, CURLOPT_NOBODY, TRUE);
		$data = curl_exec($chf);
		$size = curl_getinfo($chf, CURLINFO_CONTENT_LENGTH_DOWNLOAD);
		curl_close($chf);
		return $size;
	}
}
